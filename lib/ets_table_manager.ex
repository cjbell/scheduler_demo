defmodule ETSTableManager do
  ...
  heir, the crash will not delete the table. Hence, `EtsTableManager`
  is designed to:
  ...
  Set up your supervisor children like this:
      ...
      children = [
        ...
        worker(EtsTableManager, [YourApp.TableConsumer, [:public]])
      ]

  `EtsTableManager` will wait until your `YourApp.TableConsumer`
  ...
  ## Example
      ...
      iex> {:ok, pid} = ETSTableManager.start_link(TableConsumer, [:public])
      ...> is_pid(pid)
      true
  """
  @spec start_link(atom, list) :: {:ok, pid}
  def start_link(target_process_name, ets_options \\ [], initial_data \\ []) do
    {:ok, pid} = GenServer.start_link(__MODULE__, :ok)
    GenServer.call(pid, {:create_table, target_process_name, ets_options, initial_data})
    {:ok, pid}
  end
  ...
  def handle_call({:create_table, target_process_name, ets_options, initial_data}, _from, _state) do
    ets_options = ets_options ++ [{:heir, self, {}}]
    table = :ets.new(target_process_name, ets_options)

    if length(initial_data) > 0 do
      :ets.insert(table, initial_data)
    end
